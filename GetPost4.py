import requests
import json


class classic:
    def __init__(self):
        self.string = 'LABA4'

class Singleton(object):
    instance = None
    def __new__(cls, *args, **kwargs):
        if not isinstance(cls.instance, cls):
            cls.instance = object.__new__(cls, *args, **kwargs)
        return cls.instance
    

class Server:
    def __init__(self):
        self.startlink = 'https://dicantnik.github.io'
        self.pages = {
                '/SEVER/pages/login.html' : 'Login',
                '/SEVER/pages/landing.html' : 'landing',
                '/SEVER/pages/registration.html' : 'registration',
                '/SEVER/pages/product.html' : 'product',
                '/SEVER/pages/search.html' : 'search',
                '/SEVER/pages/basket.html' : 'basket',
                '/SEVER/pages/landing.html#sale' : 'landing-sales',
                '/SEVER/pages/landing.html#about' : 'landing-about',
                '/SEVER/pages/landing.html#contacts' : 'landing-contacts'
                }
        self.exceptions = [  
                '#sale',
                '#about',
                '#contacts',
                'data:image/png;base64,',
                'data:image/png;base64,',
                '#image0_188_56',
                '#image0_188_53',
                'https://fonts.googleapis.com',
                'https://fonts.gstatic.com',
                '/SEVER/styles/login.css',
                '/SEVER/styles/registration.css',
                '/SEVER/styles/landing.css',
                '/SEVER/styles/search.css',
                '/SEVER/styles/basket.css',
                '/SEVER/styles/product.css',
                'https://fonts.googleapis.com/css2?family=Roboto+Slab:wght@400;700&display=swap',
                'https://fonts.googleapis.com/css2?family=Roboto+Slab:wght@400;500;700&display=swap',
                'https://cdn.jsdelivr.net/npm/swiper@9/swiper-bundle.min.css'
                ]
        self.post = [
                '/SEVER/pages/registration.html',
                '/SEVER/pages/login.html',
                ]

    def runserver(self):
        choosen_link = '/SEVER/pages/landing.html'
        while True:
            link = self.startlink + choosen_link
            current_page = requests.get(link)
            print(current_page.text)

            if choosen_link in self.post:
                print('METHOD POST')
                data = input('Data: ')
                response = requests.post(link, data=json.loads(data))
                print(response)
                input('Enter to continue')
            print('\nLinks:')
            num = 1
            links = self.links(current_page)
            for link in links:
                page_name = self.pages[link]
                print(f'{num}. {page_name}')
                num += 1
            choice = input("Next page? 0 - exit: ")
            if choice == '0':
                break
            else:
                choosen_link = links[int(choice)-1]

    
    def links(self, page):
        split = page.text.split('href="')
        links = []
        for text in split:
            link = text.split('"')[0]
            if link not in self.exceptions:
                links.append(link)
        links.pop(0)
        return links


class Singleserv(Singleton, Server):
    pass



# example1 = classic()
# example2 = classic()
# server1 = Singleserv()
# server2 = Singleserv()
# print(example1 is example2)
# print(server1 is server2)


server = Singleserv()
server.runserver()